...
import it.jnrpe.services.config.xml.pojo.XMLAllow;
  ...
  private void loadServerConfig(XMLConfiguration conf) {
    List<Binding> bindings =
        conf.getServerConfig().getBindList().stream()
            .map(b -> new Binding(b.getHost(), b.getPort(), b.isSsl(),
                conf.getServerConfig().getAllowList().stream().map(XMLAllow::getIp).toList()))
            .toList();
    this.serverConfig = new ServerConfig(bindings);
  }

  private void loadCommandsConfig(XMLConfiguration conf) {
    this.commandsConfig = conf.getCommands().getCommandList().stream()
        .map(c -> new CommandConfig(c.getName(), c.getPluginName(),
            c.getArgList().stream()
                .map(arg -> String.format("%s%s %s", arg.getName().length() == 1 ? "-" : "--",
                    arg.getName(), arg.getValue()).trim())
                .reduce("", (acc, arg) -> String.join(" ", acc, arg)).trim()))
        .toList();
  }
