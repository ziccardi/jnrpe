...
public class JNRPEClient {
	...
	public final ReturnValue sendCommand(final String sCommandName,
		...
		try {
			if (!useSSL) {
				...
				sslContext.init(null, new TrustManager[] { getTrustManager() },
						new SecureRandom());
	...
	private static Group configureCommandLine() {
		...
		DefaultOption nosslOption = oBuilder.withLongName("nossl")
				.withShortName("n").withDescription("Do no use SSL").create();

		DefaultOption unknownOption = oBuilder
				.withLongName("unknown")
				.withShortName("u")
				.withDescription(
						"Make socket timeouts return an UNKNOWN "
								+ "state instead of CRITICAL").create();

		DefaultOption hostOption = oBuilder
				.withLongName("host")
				.withShortName("H")
				.withDescription(
						"The address of the host running "
								+ "the JNRPE/NRPE daemon")
				.withArgument(
		...
		DefaultOption portOption = oBuilder
				...
				.withShortName("p")
				.withDescription(
						"The port on which the daemon "
								+ "is running (default=5666)")
				.withArgument(
						aBuilder.withName("port").withMinimum(1).withMaximum(1)
								.withDefault(new Long(DEFAULT_PORT))
		...
		DefaultOption timeoutOption = oBuilder
				...
				.withShortName("t")
				.withDescription(
						"Number of seconds before connection "
								+ "times out (default=10)")
				.withArgument(
						aBuilder.withName("timeout").withMinimum(1)
								.withMaximum(1)
								.withDefault(new Long(DEFAULT_TIMEOUT))
								.withValidator(positiveInt).create()).create();
		...
		DefaultOption commandOption = oBuilder
				...
				.withDescription(
						"The name of the command that "
								+ "the remote daemon should run")
				.withArgument(
						aBuilder.withName("command").withMinimum(1)
		...
		DefaultOption argsOption = oBuilder
				...
				.withShortName("a")
				.withDescription(
						"Optional arguments that should be "
		...
		DefaultOption helpOption = oBuilder.withLongName("help")
				.withShortName("h").withDescription("Shows this help").create();

		Group executionOption = gBuilder.withOption(nosslOption)
				.withOption(unknownOption).withOption(hostOption)
				.withOption(portOption).withOption(timeoutOption)
				.withOption(commandOption).withOption(argsOption).create();

		Group mainGroup = gBuilder.withOption(executionOption)
				.withOption(helpOption).withMinimum(1).withMaximum(1).create();
	...
	private static void printVersion() {
		...
		System.out.println("jcheck_nrpe version "
				+ JNRPEClient.class.getPackage().getImplementationVersion());
	...
	public static void main(final String[] args) throws Exception {
		...
		try {
			...
			JNRPEClient client = new JNRPEClient(sHost, port.intValue(),
					!cli.hasOption("--nossl"));
			client.setTimeout(((Long) cli.getValue("--timeout", new Long(
					DEFAULT_TIMEOUT))).intValue());

			@SuppressWarnings("unchecked")
			ReturnValue ret = client.sendCommand(sCommand, (String[]) cli
					.getValues("--arglist").toArray(new String[0]));
			...
			if (timeoutAsUnknown && exc.getCause() != null
					&& exc.getCause() instanceof SocketTimeoutException) {
