...
public class BufferLengthReader implements IPacketFieldReader {
  public void read(final ByteBuf buffer, final NRPEPacket packet) {
    if (packet.getVersion() >= 3) {
      var bufferLength = buffer.readUnsignedInt();
      packet.setBufferLength(bufferLength);
      return;
    }

    packet.setBufferLength(1024);
  }
}
