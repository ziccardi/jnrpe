...
public class CheckOraclePluginNewThresholdIT implements ITConstants {
	...
	public void setup() throws Exception {
		...
		try {
			PluginDefinition checkFile = PluginRepositoryUtil
			...
			if (pce.getMessage() != null
					&& pce.getMessage().startsWith("Duplicate plugin class")) {
				// ignore: the plugin has been already instantiated...
			} else {
				throw pce;
			}
		}
		...
		cr.addCommandDefinition(new CommandDefinition("CHECK_ORACLE_ALIVE_NEW",
				...
				.addArgument(
						new CommandOption("th",
								"metric=conn,ok=$ARG5$,warn=$ARG6$,crit=$ARG7$,unit=s"))
	...
	public void checkIsAliveOk() throws JNRPEClientException {
		...
		ReturnValue ret = client.sendCommand("CHECK_ORACLE_ALIVE_NEW", "scott",
				"tiger", "mockdb", "127.0.0.1", "0..10", "10..20", "20..inf");
	...
	public void checkIsAliveWarning() throws JNRPEClientException {
		...
		ReturnValue ret = client.sendCommand("CHECK_ORACLE_ALIVE_NEW", "scott",
				"tiger", "mockdb", "127.0.0.1", "1..3", "3..5", "5..inf");
	...
	public void checkIsAliveKo() throws JNRPEClientException {
		...
		client.setTimeout(120);
		ReturnValue ret = client.sendCommand("CHECK_ORACLE_ALIVE_NEW", "scott",
				"tiger", "mockdbko", "127.0.0.1", "0..10", "10..20", "20..inf");
		Assert.assertEquals(ret.getStatus(), Status.CRITICAL, ret.getMessage());
	}
