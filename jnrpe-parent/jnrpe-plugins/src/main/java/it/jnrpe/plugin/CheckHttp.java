...
public class CheckHttp extends PluginBase {
	...
	public final void configureThresholdEvaluatorBuilder(
			...
			throws BadThresholdException {
		thrb.withLegacyThreshold("time", null, cl.getOptionValue("warning"),
	...
	public final Collection<Metric> gatherMetrics(final ICommandLine cl)
		...
		if (hostname == null) {
			throw new MetricGatheringException("No hostname specified.",
					Status.WARNING, null);
		}
		...
		String response = getHttpResponse(cl, hostname, port, method, path,
				timeout, ssl, metrics);
	 ...
	 */
	private String getHttpResponse(final ICommandLine cl,
			final String hostname, final String port, final String method,
			final String path, final int timeout, final boolean ssl,
		...
		try {
			...
			if (cl.getOptionValue("certificate") != null) {
				...
				response = checkRedirectResponse(url, method, timeout, props,
						data, redirect, ignoreBody, metrics);
			} else {
				try {
					if ("GET".equals(method)) {
						response = HttpUtils.doGET(url, props, timeout, true,
								ignoreBody);
					} else if ("POST".equals(method)) {
						response = HttpUtils.doPOST(url, props, null, data,
								true, ignoreBody);
					} else if ("HEAD".equals(method)) {
						response = HttpUtils.doHEAD(url, props, timeout, true,
								ignoreBody);
					}
					...
					log.error("Bad url", e);
					throw new MetricGatheringException("Bad url string : "
							+ urlString, Status.CRITICAL, e);
				}
			}

		} catch (Exception e) {
			log.error("Exception: " + e.getMessage(), e);
			throw new MetricGatheringException(e.getClass().getName() + ": "
					+ e.getMessage(), Status.CRITICAL, e);
		}
		return response;
	}
	 ...
	 *             -
	 */
	private String checkRedirectResponse(final URL url, final String method,
			final Integer timeout, final Properties props,
			final String postData, final String redirect,
			final boolean ignoreBody, final List<Metric> metrics)
			throws Exception {
		...
		if (!redirectedUrl.equals(initialUrl)) {
			Metric metric = new Metric("onredirect", "", new BigDecimal(1),
					null, null);
			metrics.add(metric);
		}
		return response;
	 ...
	 *             List<Metric>
	 */
	private List<Metric> analyzeResponse(final ICommandLine opt,
			final String response, final int elapsed)
			throws MetricGatheringException {
		List<Metric> metrics = new ArrayList<Metric>();
		...
		if (opt.getOptionValue("certificate") == null
			...
			if (opt.getOptionValue("string") != null
					&& !opt.getOptionValue("string").equals("")) {
				boolean found = false;
				String string = opt.getOptionValue("string");
				found = response.contains(string);
				metrics.add(new Metric("string", "", new BigDecimal(Utils
						.getIntValue(found)), null, null));
			}
			if (opt.getOptionValue("expect") != null) {
				...
				metrics.add(new Metric("expect", "" + count + " times. ",
						new BigDecimal(count), null, null));
			}
			if (opt.getOptionValue("regex") != null) {
				...
				if (opt.hasOption("invert-regex")) {
					metrics.add(new Metric("invert-regex", "" + found,
							new BigDecimal(Utils.getIntValue(found)), null,
							null));
				} else {
					metrics.add(new Metric("regex", "" + found, new BigDecimal(
							Utils.getIntValue(found)), null, null));
				}
			}
		}
		return metrics;
	...
	private Properties getRequestProperties(final ICommandLine cl,
		...
		if (cl.getOptionValue("content-type") != null
		...
		String encoded = null;
		if (cl.hasOption("authorization")) {
			encoded = Base64.encodeBase64String(cl.getOptionValue(
	...
	private String getPostData(final ICommandLine cl) throws Exception {
		...
		for (String value : values) {
			...
			encoded.append(key).append("=")
					.append(URLEncoder.encode(val, "UTF-8")).append("&");
		}

		String res = encoded.toString();
	...
	private void checkCertificateExpiryDate(URL url, List<Metric> metrics)
			throws Exception {
		...
		for (Date date : expiryDates) {
			...
			metrics.add(new Metric("certificate", "",
					new BigDecimal(diffInDays), null, null));
		}
	}
