...
public class CheckHttp extends PluginBase {
    ...
    private String getHttpResponse(
        ...
        try {
            ...
            if (cl.hasOption("authorization")) {
                urlString =
                        cl.getOptionValue("authorization") + "@" + urlString;
            } else if (cl.hasOption("proxy-authorization")) {
                urlString =
                        cl.getOptionValue("proxy-authorization") + "@"
                                + urlString;
            }
            if (ssl) {
            ...
            if (cl.getOptionValue("certificate") != null) {
                ...
                response =
                        checkRedirectResponse(url, method, timeout, props,
                                data, redirect, ignoreBody, metrics);
            } else {
                try {
                    if ("GET".equals(method)) {
                        response =
                                HttpUtils.doGET(url, props, timeout, true,
                                        ignoreBody);
                    } else if ("POST".equals(method)) {
                        response =
                                HttpUtils.doPOST(url, props, null, data, true,
                                        ignoreBody);
                    } else if ("HEAD".equals(method)) {
                        response =
                                HttpUtils.doHEAD(url, props, timeout, true,
                                        ignoreBody);
                    }
     ...
     *             -
     */
    private String checkRedirectResponse(
            final URL url,
            final String method,
            ...
            final List<Metric> metrics) throws Exception {
        ...
        if (!redirectedUrl.equals(initialUrl)) {
            Metric metric =
                    new Metric("onredirect", "", new BigDecimal(1), null, null);
            metrics.add(metric);
        }
        return response;
    }
     ...
     *             List<Metric>
     */
    private List<Metric> analyzeResponse(
            final ICommandLine opt,
            final String response,
            final int elapsed) throws MetricGatheringException {
        List<Metric> metrics = new ArrayList<Metric>();
        metrics.add(new Metric("time", "",
                new BigDecimal(elapsed), null, null));
    ...
    private void checkCertificateExpiryDate(URL url, List<Metric> metrics)
        ...
        for (Date date : expiryDates) {
            int diffInDays =
                    (int) ((date.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
