...
public class JNRPEClient {
    ...
    private static Group configureCommandLine() {
        ...
        DefaultOption nosslOption =
                oBuilder.withLongName("nossl").withShortName("n")
                        .withDescription("Do no use SSL").create();

        DefaultOption unknownOption =
                oBuilder.withLongName("unknown")
                        .withShortName("u")
                        .withDescription(
        ...
        DefaultOption hostOption =
                oBuilder.withLongName("host")
                        .withShortName("H")
                        .withDescription(
                                "The address of the host running "
                                        + "the JNRPE/NRPE daemon")
                        .withArgument(
                                aBuilder.withName("host").withMinimum(1)
                                        .withMaximum(1).create()).create();

        NumberValidator positiveInt = NumberValidator.getIntegerInstance();
        positiveInt.setMinimum(0);
        DefaultOption portOption =
                oBuilder.withLongName("port")
                        .withShortName("p")
                        .withDescription(
                                "The port on which the daemon "
                                        + "is running (default=5666)")
                        .withArgument(
                                aBuilder.withName("port").withMinimum(1)
                                        .withMaximum(1)
                                        .withDefault(new Long(DEFAULT_PORT))
                                        .withValidator(positiveInt).create())
                        .create();

        DefaultOption timeoutOption =
                oBuilder.withLongName("timeout")
                        .withShortName("t")
                        .withDescription(
        ...
        DefaultOption argsOption =
                oBuilder.withLongName("arglist")
                        .withShortName("a")
                        .withDescription(
                        ...
                        .withArgument(
                                aBuilder.withName("arglist").withMinimum(1)
                                        .create()).create();
        ...
        Group executionOption =
                gBuilder.withOption(nosslOption).withOption(unknownOption)
                        .withOption(hostOption).withOption(portOption)
                        .withOption(timeoutOption).withOption(commandOption)
                        .withOption(argsOption).create();

        Group mainGroup =
                gBuilder.withOption(executionOption).withOption(helpOption)
    ...
    public static void main(final String[] args) throws Exception {
        ...
        try {
            ...
            JNRPEClient client =
                    new JNRPEClient(sHost, port.intValue(),
                            !cli.hasOption("--nossl"));
            ...
            ReturnValue ret =
                    client.sendCommand(
                            sCommand,
                            (String[]) cli.getValues("--arglist").toArray(
                                    new String[0]));
