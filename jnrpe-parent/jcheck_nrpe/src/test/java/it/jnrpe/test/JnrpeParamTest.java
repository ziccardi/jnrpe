...
public class JnrpeParamTest {
    ...
    public void setUp() throws Exception {
        ...
        PluginDefinition pd =
                new PluginDefinition("TESTPLUGIN", "This is a test plugin",
                        new TestPlugin()).addOption(new PluginOption()
                        .setRequired(true).setOption("t").setLongOpt("type")
                        .setHasArgs(true));
        pr.addPluginDefinition(pd);

        CommandDefinition cd =
                new CommandDefinition("TESTCOMMAND", "TESTPLUGIN")
            ...
            instance2.addEventListener(new IJNRPEEventListener() {
                ...
                public void receive(Object sender, IJNRPEEvent event) {
                    if (event.getEventName().equals(LogEvent.ERROR.name())) {
                        ...
                        m_sEventMessage =
                                (String) event.getEventParams().get("MESSAGE");
                    }
                }
            });
    ...
    public void testCommandNullPointerException() throws Exception {
        JNRPEClient client = new JNRPEClient("127.0.0.1", 5667, false);
        ReturnValue ret =
                client.sendCommand("TESTCOMMAND", "NullPointerException");
    ...
    public void testThrowRuntimeException() throws Exception {
        JNRPEClient client = new JNRPEClient("127.0.0.1", 5667, false);
        ReturnValue ret =
                client.sendCommand("TESTCOMMAND", "ThrowRuntimeException");
        Assert.assertEquals(ret.getStatus(), Status.UNKNOWN);
        Assert.assertEquals(
                ret.getMessage()
