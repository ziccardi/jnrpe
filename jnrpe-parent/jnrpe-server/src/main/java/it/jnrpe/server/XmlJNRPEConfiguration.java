...
class XmlJNRPEConfiguration extends JNRPEConfiguration {
    ...
    public void load(final File confFile) throws ConfigurationException {
        ...
        try {
            ...
            List<Object> vBindAddresses =
                    confParser.getList("server.bind[@address]");
            int iBindListSize = vBindAddresses.size();

            for (int i = 0; i < iBindListSize; i++) {
                String sAddress =
                        confParser
                                .getString("server.bind(" + i + ")[@address]");
                String useSSL =
                        confParser.getString("server.bind(" + i + ")[@SSL]");
                if (useSSL == null) {
                    useSSL = "false";
                }
            ...
            String sAcceptParams =
                    confParser.getString("server[@accept-params]", "true");
            ...
            List<Object> vAllowedAddresses =
                    confParser.getList("server.allow[@ip]");
            ...
            for (int i = 0; i < iCount; i++) {
                String sCommandName =
                        (String) confParser.getProperty("commands.command(" + i
                                + ")[@name]");
                String sPluginName =
                        (String) confParser.getProperty("commands.command(" + i
                                + ")[@plugin_name]");
                String sWholeCommandLine =
                        (String) confParser.getProperty("commands.command(" + i
                                + ")[@params]");
                ...
                Object argsObj =
                        confParser.getProperty("commands.command(" + i
                                + ").arg[@name]");
                int iArgsCount = 0;
                if (argsObj != null) {
                ...
                StringBuffer commandLineBuffer =
                        new StringBuffer(sWholeCommandLine);

                for (int j = 0; j < iArgsCount; j++) {
                    String sArgName =
                            (String) confParser.getProperty("commands.command("
                                    + i + ").arg(" + j + ")[@name]");
                    String sArgValue =
                            (String) confParser.getProperty("commands.command("
                                    + i + ").arg(" + j + ")[@value]");
