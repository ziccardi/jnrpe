...
public final class JNRPEServer {
	...
	private static Group configureCommandLine() {
		...
		DefaultOption listOption = oBuilder.withLongName("list")
				.withShortName("l")
		...
		DefaultOption helpOption = oBuilder.withLongName("help")
				.withShortName("h").withDescription("Show this help").create();
		...
		DefaultOption pluginHelpOption = oBuilder
				.withLongName("help")
				.withShortName("h")
		...
		Group alternativeOptions = gBuilder.withOption(listOption)
				.withOption(pluginHelpOption).create();

		DefaultOption confOption = oBuilder
				.withLongName("conf")
				.withShortName("c")
		...
		DefaultOption interactiveOption = oBuilder.withLongName("interactive")
				.withShortName("i")
				.withDescription("Starts JNRPE in command line mode").create();

		Group jnrpeOptions = gBuilder.withOption(confOption)
				.withOption(interactiveOption).withMinimum(1).create();

		Group mainGroup = gBuilder.withOption(versionOption)
				.withOption(helpOption).withOption(jnrpeOptions).create();
	...
	private static void printHelp(final IPluginRepository pr,
		...
		try {
			...
			if (pp == null) {
				System.out
						.println("Plugin " + pluginName + " does not exists.");
			} else {
				pp.printHelp();
			}
		} catch (Exception e) {
	 ...
	 *             -
	 */
	private static IPluginRepository loadPluginDefinitions(
			final String sPluginDirPath) throws PluginConfigurationException {
	...
	public static void main(final String[] args) {
		...
		try {
			...
			System.out.println("Unable to parse the configuration at "
					+ cl.getValue("--conf") + ". The error is : "
					+ e.getMessage());
			e.printStackTrace();
			System.exit(-1);
		}
		...
		try {
			...
			System.out.println("An error has occurred while parsing "
