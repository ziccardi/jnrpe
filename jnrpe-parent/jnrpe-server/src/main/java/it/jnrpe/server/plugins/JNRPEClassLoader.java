...
class JNRPEClassLoader extends ClassLoader {
	...
	private static class ChildURLClassLoader extends URLClassLoader {
		 ...
		 */
		public ChildURLClassLoader(final URL[] urls,
				final FindClassClassLoader parent) {
			super(urls, null);
	...
	public JNRPEClassLoader(final List<URL> classpath) {
		...
		childClassLoader = AccessController
				.doPrivileged(new PrivilegedAction<ChildURLClassLoader>() {
	...
	protected URL findResource(final String name) {
		...
		try {
			for (URL u : classLoaderUrlList) {
				...
				String resourceUrl = "jar:" + new File(u.getFile()).toURI()
						+ "!/" + name;
